...
unsigned int mct_offline_trace_storage_dir_info(char *path, char *file_name, char *newest, char *oldest)
{
    ...
    if ((path == NULL) || (file_name == NULL) || (newest == NULL) || (oldest == NULL)) {
        printf("mct_offline_trace_storage_dir_info: Invalid parameter(s)");
        return 0;
    }
...
ssize_t mct_offline_trace_get_total_size(DltOfflineTrace *trace)
{
    ...
    /* go through all mct files in directory */
    ...
    while ((dp = readdir(dir)) != NULL)
        if (strstr(dp->d_name, DLT_OFFLINETRACE_FILENAME_BASE)) {
            ...
            /*else */
            /*{ */
            /*    mct_log(3, "mct_offline_trace_get_total_size: long filename ignored"); */
            /*} */
        }
                                      ...
                                      int filenameTimestampBased)
{
    ...
    mct_offline_trace_check_size(trace);
                                       ...
                                       int size3)
{
    ...
    if ((lseek(trace->ohandle, 0, SEEK_CUR) + size1 + size2 + size3) >= trace->fileSize) {
        ...
        /* create new file */
        mct_offline_trace_create_new_file(trace);
    }
