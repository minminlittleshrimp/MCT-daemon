 ...
 */
int mct_env_helper_to_lower(char **const env, char *result, int const res_len)
{
    int count = 0;
    char ch;
...
int mct_env_extract_symbolic_ll(char **const env, int8_t *ll)
{
    ...
    if (mct_env_helper_to_lower(env, &result[0], sizeof(result)) == 0) {
...
int mct_env_extract_ll(char **const env, int8_t *ll)
{
    ...
    if (**env == '-') {
        ...
        if ((**env >= '0') && (**env < '7')) {
            *ll = **env - '0';
            (*env)++;
        } else if (mct_env_extract_symbolic_ll(env, ll) != 0) {
            return -1;
        }
    }
 ...
 * @return 0 if successful, -1 else
 */
int mct_env_extract_ll_item(char **const env, mct_env_ll_item *const item)
{
    int ret = -1;
    ...
    memset(item, 0, sizeof(mct_env_ll_item));
    ret = mct_env_extract_id(env, item->appId);
    ...
    (*env)++;
    ret = mct_env_extract_id(env, item->ctxId);
    ...
    (*env)++;
    ret = mct_env_extract_ll(env, &item->ll);
 ...
 * @return 0 on success
 */
int mct_env_init_ll_set(mct_env_ll_set *const ll_set)
{
    if (!ll_set) {
        return -1;
    }
 ...
 * @return 0 on success
 */
int mct_env_increase_ll_set(mct_env_ll_set *const ll_set)
{
    mct_env_ll_item *old_set;
    size_t old_size;
...
int mct_env_extract_ll_set(char **const env, mct_env_ll_set *const ll_set)
{
    ...
    if (mct_env_init_ll_set(ll_set) == -1) {
        return -1;
    }

    do {
        if (ll_set->num_elem == ll_set->array_size) {
            if (mct_env_increase_ll_set(ll_set) == -1) {
                return -1;
            }
        }

        if (mct_env_extract_ll_item(env, &ll_set->item[ll_set->num_elem++]) == -1) {
            return -1;
        }
 ...
 */
int mct_env_ids_match(char const *const a, char const *const b)
{
    if (a[0] != b[0]) {
        return 0;
    }
 ...
 */
int mct_env_ll_item_get_matching_prio(mct_env_ll_item const *const item,
                                      ...
                                      char const *const ctid)
{
    ...
    if (item->appId[0] == 0) {
        if (item->ctxId[0] == 0) {
            return 1;
        } else if (mct_env_ids_match(item->ctxId, ctid)) {
            return 2;
        }
    } else if (mct_env_ids_match(item->appId, apid)) {
        if (item->ctxId[0] == 0) {
            return 3;
        } else if (mct_env_ids_match(item->ctxId, ctid)) {
            return 4;
                               ...
                               int const ll)
{
    ...
    for (i = 0; i < ll_set->num_elem; ++i) {
        int p = mct_env_ll_item_get_matching_prio(&ll_set->item[i], apid, ctid);
