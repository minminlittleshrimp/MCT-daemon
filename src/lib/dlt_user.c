...
static bool dlt_user_app_unreg = true;
    ...
    if (dlt_user.dlt_log_handle != -1) {
        ...
        if (result < 0) {
            dlt_vlog(LOG_WARNING,
                     ...
                     __func__, dltSockBaseDir, strerror(errno));
...
DltReturnValue dlt_init(void)
{
    ...
    */
    ...
    if (dlt_start_threads(DLT_USER_HOUSEKEEPER_THREAD) < 0) {
        dlt_user_initialised = false;
        return DLT_RETURN_ERROR;
    }
        ...
        if (ret < 0) {
            ...
            while (1) {
                ...
                if (ret < 0) {
                    ...
                    bytes_read = read(dlt_user.dlt_log_handle,
...
DltReturnValue dlt_register_app(const char *apid, const char *description)
{
    ...
    if (p_app_id[1] == 0) {
        if (p_app_id[0] == dlt_user.appID[0]) {
            return DLT_RETURN_OK;
        }
    } else if (p_app_id[2] == 0) {
        if ((p_app_id[0] == dlt_user.appID[0]) &&
            (p_app_id[1] == dlt_user.appID[1])) {
            return DLT_RETURN_OK;
        }
    } else if (p_app_id[3] == 0) {
        if ((p_app_id[0] == dlt_user.appID[0]) &&
            (p_app_id[1] == dlt_user.appID[1]) &&
            (p_app_id[2] == dlt_user.appID[2])) {
            return DLT_RETURN_OK;
        }
    } else {
        if ((p_app_id[0] == dlt_user.appID[0]) &&
    ...
    dlt_set_id(dlt_user.appID, p_app_id);
...
DltReturnValue dlt_register_injection_callback_with_id(DltContext *handle,
                                                       uint32_t service_id,
                                                       dlt_injection_callback_id dlt_injection_cbk,
                                                       void *priv)
{
    DltContextData log;
    uint32_t i, j, k;
    ...
    if (priv == NULL) {
        dlt_user.dlt_ll_ts[i].injection_table[j].injection_callback =
            (dlt_injection_callback)(void *)dlt_injection_cbk;
                                                                             ...
                                                                             uint32_t length))
{
    return dlt_register_injection_callback_with_id(
               handle,
               service_id,
               (dlt_injection_callback_id)(void *)
               dlt_injection_callback,
               NULL);
}
                        ...
                        if (usercontextll != NULL) {
                            ...
                            if ((usercontextll->log_level_pos >= 0) &&
                                ...
                                if (dlt_user.dlt_ll_ts) {
                                    ...
                                    if (dlt_user.dlt_ll_ts[usercontextll->log_level_pos].
                                        log_level_ptr) {
                    ...
                    case DLT_USER_MESSAGE_INJECTION:
                    {
                        ...
                        if (userbuffer != NULL) {
                            ...
                            if ((usercontextinj->data_length_inject > 0) && (dlt_user.dlt_ll_ts)) {
                                ...
                                for (i = 0;
                                     i <
                                     dlt_user.dlt_ll_ts[usercontextinj->log_level_pos].nrcallbacks;
                                     i++) {
                                    if ((dlt_user.dlt_ll_ts[usercontextinj->log_level_pos].
                                        ...
                                        if (dlt_user.dlt_ll_ts[usercontextinj->log_level_pos].
                                            ...
                                            delayed_injection_callback.injection_callback =
                                                ...
                                                injection_table[i].
                                                injection_callback;
                                        } else if (dlt_user.dlt_ll_ts[usercontextinj->log_level_pos
                                                   ].injection_table[i].
...
DltReturnValue dlt_user_log_resend_buffer(void)
{
    ...
    for (num = 0; num < count; num++) {
        ...
        DLT_SEM_LOCK();
        size = dlt_buffer_copy(&(dlt_user.startup_buffer),
                               dlt_user.resend_buffer,
                               dlt_user.log_buf_len);
        ...
        if (size > 0) {
            ...
            if (dlt_user_check_userheader(userheader)) {
                switch (userheader->message) {
                    ...
                    {
                        DltUserControlMsgRegisterContext *usercontext =
                            (DltUserControlMsgRegisterContext *)(dlt_user.resend_buffer +
                                                                 sizeof(DltUserHeader));
...
{
    if ((dlt_housekeeperthread_handle == 0) && (id & DLT_USER_HOUSEKEEPER_THREAD)) {
        /* Start housekeeper thread */
        if (pthread_create(&dlt_housekeeperthread_handle, 0,
                           (void *)&dlt_user_housekeeperthread_function, 0) != 0) {
            dlt_log(LOG_CRIT, "Failed to create housekeeper thread!\n");
            return -1;
        }
    }
    return 0;
