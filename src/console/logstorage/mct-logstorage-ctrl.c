...
#define MCT_LOGSTORAGE_CTRL_EXIT 1
 ...
 */
void mct_logstorage_exit(void)
{
    must_exit = MCT_LOGSTORAGE_CTRL_EXIT;
}
 ...
 * The application will exit on next poll timeout.
 */
int mct_logstorage_must_exit(void)
{
    return must_exit;
}
 ...
 */
static int mct_logstorage_ctrl_add_event(struct mct_event *ev_hdl,
                                         int fd,
                                         void *cb)
{
    if ((fd < 0) || !cb || !ev_hdl) {
 ...
 * @return 0 on success, -1 otherwise.
 */
static int mct_logstorage_ctrl_execute_event_loop(struct mct_event *ev)
{
    int ret = 0;
...
/** @brief Start event loop and receive messages from MCT.
 *
 ...
 */
static int mct_logstorage_ctrl_setup_event_loop(void)
{
    int ret = 0;
    struct mct_event ev_hdl = {
        .pfd = {
            .fd = -1,
            .events = POLLIN
        }
    ...
    if (mct_logstorage_init_handler() < 0) {
        pr_error("Failed to initialize handler.\n");
        mct_control_deinit();
        return -1;
    }

    if (mct_logstorage_ctrl_add_event(&ev_hdl,
                                      mct_logstorage_get_handler_fd(),
                                      mct_logstorage_get_handler_cb()) < 0) {
        pr_error("add_event error: %s\n", strerror(errno));
        mct_logstorage_exit();
    }

    while (!mct_logstorage_must_exit() && (ret == 0))
        ret = mct_logstorage_ctrl_execute_event_loop(&ev_hdl);

    /* Clean up */
    mct_logstorage_deinit_handler();
    mct_control_deinit();

    return ret;
}

/** @brief Send a single command to MCT daemon and wait for response
 *
 * @return 0 on success, -1 otherwise.
 */
static int mct_logstorage_ctrl_single_request()
{
    int ret = 0;
    ...
    if (get_default_event_type() != EVENT_SYNC_CACHE) {
        ...
        if (!mct_logstorage_check_config_file(get_default_path())) {
        ...
        if (!mct_logstorage_check_directory_permission(get_default_path())) {
    ...
    ret = mct_logstorage_send_event(get_default_event_type(),
                                    get_default_path());
...
{
    printf("Usage: mct-logstorage-ctrl [options]\n");
    printf("Send a trigger to MCT daemon to connect/disconnect"
    ...
    printf("  -C filename                MCT daemon configuration file (Default: " CONFIGURATION_FILES_DIR
           "/mct.conf)\n");
}
...
static int parse_args(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc,
        ...
        case 's':
            ...
            if ((optarg != NULL) && (strlen(optarg) >= MCT_MOUNT_PATH_MAX)) {
        ...
        case 'p':
            ...
            if (strlen(optarg) >= MCT_MOUNT_PATH_MAX) {
                pr_error("Mount path '%s' too long\n", optarg);
                return -1;
            }
...
int main(int argc, char *argv[])
{
    ...
    if (get_handler_type() == CTRL_NOHANDLER) {
        ...
        ret = mct_logstorage_ctrl_single_request();

        if (ret < 0)
            pr_error("Message failed to be send. Please check MCT config.\n");
    }
    else {
        ...
        ret = mct_logstorage_ctrl_setup_event_loop();
    }
