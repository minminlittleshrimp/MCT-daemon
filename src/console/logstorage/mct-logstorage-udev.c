 ...
 */
static char *mct_logstorage_udev_get_mount_point(char *dev_node)
{
    struct mntent *ent;
    FILE *f;
    char *mnt_point = NULL;
 ...
 *
 * On mount event:
 * If the device mount point contains the MCT configuration file,
...
static int check_mountpoint_from_partition(int event, struct udev_device *part)
{
    ...
    if (event == EVENT_MOUNTED) {
        mnt_point = mct_logstorage_udev_get_mount_point(dev_node);
        logstorage_dev = mct_logstorage_check_config_file(mnt_point);
    ...
    if (mnt_point) {
        ret = mct_logstorage_send_event(event, mnt_point);

        if (ret)
            pr_error("Can't send event for %s to MCT.\n", mnt_point);
    }
 ...
 */
static int mct_logstorage_udev_check_mounted(struct udev *udev)
{
    if (udev == NULL) {
...
int mct_logstorage_udev_init(void)
{
    ...
    return mct_logstorage_udev_check_mounted(prvt->udev);
}
