...
typedef struct
{
    ...
    char cvalue[NAME_MAX + 1];                          /**< (String: Directory) Filename of DLT configuration file (Default: /etc/mct.conf) */
    ...
    char offlineTraceDirectory[DLT_DAEMON_FLAG_MAX];    /**< (String: Directory) Store DLT messages to local directory (Default: /etc/mct.conf) */
    ...
    char userPipesDir[DLT_PATH_MAX];    /**< (String: Directory) directory where mctpipes reside (Default: /tmp/mctpipes) */
    char daemonFifoName[DLT_PATH_MAX];  /**< (String: Filename) name of local fifo (Default: /tmp/mct) */
...
typedef struct
{
    ...
    DltMessage msg;                  /**< one mct message */
...
int mct_daemon_local_connection_init(DltDaemon *daemon, DltDaemonLocal *daemon_local, int verbose);
...
int mct_daemon_process_user_messages(DltDaemon *daemon,
                                     ...
                                     DltReceiver *recv,
                                     int verbose);
int mct_daemon_process_one_s_timer(DltDaemon *daemon,
                                   DltDaemonLocal *daemon_local,
                                   DltReceiver *recv,
                                   int verbose);
int mct_daemon_process_sixty_s_timer(DltDaemon *daemon,
                                     DltDaemonLocal *daemon_local,
                                     DltReceiver *recv,
                                     int verbose);
int mct_daemon_process_systemd_timer(DltDaemon *daemon,
                                     DltDaemonLocal *daemon_local,
                                     DltReceiver *recv,
                                     int verbose);

int mct_daemon_process_control_connect(DltDaemon *daemon,
                                       DltDaemonLocal *daemon_local,
                                       DltReceiver *recv,
                                       int verbose);
#if defined DLT_DAEMON_USE_UNIX_SOCKET_IPC
int mct_daemon_process_app_connect(DltDaemon *daemon,
                                   DltDaemonLocal *daemon_local,
                                   DltReceiver *recv,
                                   int verbose);
#endif
int mct_daemon_process_control_messages(DltDaemon *daemon,
                                        DltDaemonLocal *daemon_local,
                                        DltReceiver *recv,
                                        int verbose);

typedef int (*mct_daemon_process_user_message_func)(DltDaemon *daemon, DltDaemonLocal *daemon_local,
                                                    DltReceiver *rec,
                                                    int verbose);
...
void mct_daemon_systemd_watchdog_thread(void *ptr);
#endif
...
int mct_daemon_client_update(DltDaemon *daemon, DltDaemonLocal *daemon_local, int verbose);
#endif /* DLT_DAEMON_H */
