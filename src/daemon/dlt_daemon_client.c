                           ...
                           int verbose)
{
    ...
    if ((sock != DLT_DAEMON_SEND_FORCE) && (daemon->state != DLT_DAEMON_STATE_SEND_BUFFER)) {
        if ((client_mask & DLT_CONNECTION_TO_MASK(DLT_CONNECTION_CLIENT_MSG_OFFLINE_TRACE)) &&
            ...
            if (dlt_offline_trace_write(&(daemon_local->offlineTrace), storage_header,
                                      ...
                                      int verbose)
{
    ...
    DltConnection *con =
        dlt_event_handler_find_connection(&daemon_local->pEvent, sock);
    ...
    if ((id > DLT_SERVICE_ID) && (id < DLT_SERVICE_ID_CALLSW_CINJECTION)) {
        ...
        switch (id) {
            ...
            {
                if (dlt_daemon_applications_save(daemon, daemon->runtime_application_cfg,
                                                 verbose) == 0) {
                    if (dlt_daemon_contexts_save(daemon, daemon->runtime_context_cfg,
                                                 verbose) == 0) {
            ...
            {
                dlt_daemon_control_set_default_trace_status(sock,
                                                            daemon,
                                                            daemon_local,
                                                            msg,
                                                            verbose);
            ...
            {
                dlt_daemon_control_message_buffer_overflow(sock,
                                                           daemon,
                                                           daemon_local,
                                                           daemon->overflow_counter,
                                                           "",
            ...
            {
                dlt_daemon_control_set_filter_level(sock, daemon, daemon_local,
                                                    msg, verbose);
                break;
            }
            case DLT_SERVICE_ID_GET_FILTER_STATUS:
            {
                dlt_daemon_control_get_filter_status(sock, daemon, daemon_local,
                                                     verbose);
                break;
            }
                                               ...
                                               int verbose)
{
    ...
    /* send message */
    if ((ret =
             dlt_daemon_client_send_control_message(sock, daemon, daemon_local, &msg, apid, "",
                                                    verbose))) {
        dlt_message_free(&msg, 0);
        return ret;
    }
                                                            ...
                                                            int verbose)
{
    ...
    for (count = 0; count < user_list->num_contexts; count++) {
        ...
        if (context) {
            ...
            if (ret == 0) {
                dlt_daemon_send_trace_status(sock,
                                             daemon,
                                             daemon_local,
                                             context,
                                             tracestatus,
...
void dlt_daemon_control_set_filter_level(int sock, DltDaemon *daemon,
                                         DltDaemonLocal *daemon_local,
                                         DltMessage *msg, int verbose)
{
    ...
    if (dlt_daemon_filter_change_filter_level(daemon_local, new_level,
                                              verbose) < 0) {
        dlt_daemon_control_service_response(sock, daemon, daemon_local, id,
...
void dlt_daemon_control_get_filter_status(int sock, DltDaemon *daemon,
                                          DltDaemonLocal *daemon_local,
                                          int verbose)
{
    PRINT_FUNCTION_VERBOSE(verbose);
